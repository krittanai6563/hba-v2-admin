...
// Interface นี้ใช้สำหรับเก็บข้อมูลที่ "รวบรวมยอด" แล้ว
...
// ref นี้จะเก็บข้อมูลที่ "รวบรวมยอด" แล้ว
...
// ฟังก์ชันนี้รับข้อมูลที่ "รวบรวมยอด" แล้ว
const updateChartData = (data: SummaryData) => {
    const categories = categoryOrder;

    // (บรรทัด 167 เดิม) ตอนนี้ data.unit จะถูกการันตีว่ามีอยู่
...
const fetchSummary = async () => {
    ...
    try {
        const res = await fetch('https://uat.hba-sales.org/backend/get_contract_summary_main.php', {
            ...
            credentials: 'include', // (จำเป็นหากมีการตรวจสอบ Session)
            body: JSON.stringify({
                user_id: userId,
                buddhist_year: selectedYear.value,
                // ส่ง 'quarter' (ชื่อ) และ 'month' (ตัวเลข)
        ...
        // data คือข้อมูลที่ PHP ส่งมา (มีโครงสร้างตามภาค)
        const data = await res.json(); 

        // --- ✅ เริ่มการรวบรวมข้อมูล (Aggregation) ---
        // (นี่คือส่วนที่แก้ไข Error ครับ)

        // 1. สร้าง Object ว่างสำหรับเก็บยอดรวม
        const aggregatedData: SummaryData = {
            unit: { total: 0 },
            total_value: { total: 0 },
            usable_area: { total: 0 },
            price_per_sqm: { total: 0 }
        };

        // 2. กำหนดค่าเริ่มต้น 0 ให้กับทุก priceRange
        for (const range of priceRanges) {
            aggregatedData.unit[range] = 0;
            aggregatedData.total_value[range] = 0;
            aggregatedData.usable_area[range] = 0;
            aggregatedData.price_per_sqm[range] = 0;
        }

        let totalAreaForAvg = 0;
        let totalValueForAvg = 0;
        const regions = Object.keys(data); // ดึงชื่อภาคทั้งหมดจาก data

        // 3. วนลูปทุกภาค และทุกช่วงราคา เพื่อบวกค่ายอดรวม
        for (const region of regions) {
            for (const range of priceRanges) {
                // ป้องกัน Error หาก region หรือ range ไม่มีใน data
                if (data[region] && data[region][range]) {
                    const metrics = data[region][range];
                    
                    // บวกยอดเข้าแต่ละ range
                    aggregatedData.unit[range] += metrics.unit;
                    aggregatedData.total_value[range] += metrics.value; // 'value' จาก PHP
                    aggregatedData.usable_area[range] += metrics.area;   // 'area' จาก PHP

                    // บวกรวม 'total' (สำหรับการ์ดสรุป)
                    aggregatedData.unit['total'] += metrics.unit;
                    aggregatedData.total_value['total'] += metrics.value;
                    aggregatedData.usable_area['total'] += metrics.area;

                    // เก็บยอดรวมทั้งหมดเพื่อหาค่าเฉลี่ย
                    totalValueForAvg += metrics.value;
                    totalAreaForAvg += metrics.area;
                }
            }
        }

        // 4. คำนวณ 'ราคาเฉลี่ย/ตร.ม.' (แบบถ่วงน้ำหนัก)
        for (const range of priceRanges) {
            if (aggregatedData.usable_area[range] > 0) {
                aggregatedData.price_per_sqm[range] = Math.round(
                    aggregatedData.total_value[range] / aggregatedData.usable_area[range]
                );
            } else {
                aggregatedData.price_per_sqm[range] = 0;
            }
        ...
        // 5. คำนวณ 'ราคาเฉลี่ย/ตร.ม.' (สำหรับ 'total' ของการ์ดสรุป)
        if (totalAreaForAvg > 0) {
            aggregatedData.price_per_sqm['total'] = Math.round(totalValueForAvg / totalAreaForAvg);
        } else {
            aggregatedData.price_per_sqm['total'] = 0;
        }

        // --- จบการรวบรวมข้อมูล ---

        // 6. ใช้ข้อมูลที่รวบรวมแล้ว
        summaryData.value = aggregatedData; 
        
        console.log('✅ ข้อมูลที่ได้จาก backend (แยกตามภาค):', data); // (บรรทัด 215)
        console.log('✅ ข้อมูลที่ประมวลผลแล้ว (รวบรวมทุกภาค):', aggregatedData);

        // (บรรทัด 217) ส่งข้อมูลที่ "รวบรวมแล้ว" (aggregatedData) ไปให้ชาร์ต
        updateChartData(aggregatedData); 

    } catch (err) {
        console.error('❌ Error fetching summary:', err); // (บรรทัด 219)
    }
};
// =================================================================
// ✅ จบส่วนที่แก้ไข
...
// ฟังก์ชันนี้ทำงานได้เหมือนเดิม เพราะ summaryData.value มีโครงสร้างที่ถูกต้องแล้ว
