...
const fetchSummary = async () => {
    ...
    try {
        const res = await fetch('https://6e9fdf451a56.ngrok-free.app/package/backend/get_contract_summary.php', {
            method: 'POST',
...
const exportToExcel = () => {
  ...
  const chartData = chartSeries.value.map((series, index) => {
    return {
      Category: categoryOrder,
      // Explicitly type the value as a number and format with commas
      Value: series.data.map((value: number) => value.toLocaleString('th-TH')) 
    };
  });

const summaryDataArray = priceRanges.map(range => ({
    'Price Range': range,
    'จำนวนหลัง': getValue('unit', range),  // This is still a number, toLocaleString is used later
    'มูลค่ารวม': getValue('total_value', range),
    'พื้นที่ใช้สอย': getValue('usable_area', range), 
    'ราคาเฉลี่ย/ตร.ม.': getValue('price_per_sqm', range)
}));


//   // Add the formatted numbers to the array to display with commas
//   summaryDataArray.forEach(item => {
//     item['จำนวนหลัง'] = (item['จำนวนหลัง'] as number).toLocaleString('th-TH');
//     item['มูลค่ารวม'] = (item['มูลค่ารวม'] as number).toLocaleString('th-TH');
//     item['พื้นที่ใช้สอย'] = (item['พื้นที่ใช้สอย'] as number).toLocaleString('th-TH');
//     item['ราคาเฉลี่ย/ตร.ม.'] = (item['ราคาเฉลี่ย/ตร.ม.'] as number).toLocaleString('th-TH');
//   });

  const headerRow = [
    ['สรุปข้อมูลรายงานแบ่งตามมูลค่า'],
    [`ประจำปี ${selectedYear.value} - ${selectedQuarter.value || ''} ${selectedMonth.value || ''}`],
    [] 
  ];

  const headerSheet = XLSX.utils.aoa_to_sheet(headerRow);
  const summaryDataSheet = XLSX.utils.json_to_sheet(summaryDataArray);

  // Set column widths before adding the data
  summaryDataSheet['!cols'] = [
    { wpx: 200 },  // Price Range column
    { wpx: 120 },  // จำนวนหลัง column
    { wpx: 120 },  // มูลค่ารวม column
    { wpx: 120 },  // พื้นที่ใช้สอย column
    { wpx: 120 }   // ราคาเฉลี่ย/ตร.ม. column
  ];

  // Ensure data is added to the sheet correctly
  XLSX.utils.sheet_add_json(headerSheet, summaryDataArray, { origin: -1 });

  const finalWorkbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(finalWorkbook, headerSheet, "Summary Data");

  // Write the Excel file
  XLSX.writeFile(finalWorkbook, 'contract_summary_report.xlsx');
};
      ...
      <VCard elevation="10">
          ...
          <div class="v-row">
            <div class="v-col-md-8 v-col-12">
              <div class="d-flex align-center">
                <div>
                  <h3 class="card-title mb-1">รายงานยอดแบ่งตามมูลค่าบ้าน ประจำปี {{ selectedYear }} -
                                            <span v-if="selectedQuarter && !selectedMonth">{{ selectedQuarter }}</span>
                                            <span v-if="selectedMonth && !selectedQuarter">เดือน {{ selectedMonth
                                                }}</span>
                                            <span v-if="!selectedQuarter && !selectedMonth">ทั้งปี</span></h3>
        ...
        <v-col cols="12" sm="12" lg="12">
            ...
            <div class="v-col-md-8 v-col-12">
              <div class="d-flex align-center">
                <div>
                 <h3 class="card-title mb-1">ตารางสรุปยอดแบ่งตามมูลค่าบ้าน ประจำปี {{ selectedYear }} -
                                            <span v-if="selectedQuarter && !selectedMonth">{{ selectedQuarter }}</span>
                                            <span v-if="selectedMonth && !selectedQuarter">เดือน {{ selectedMonth
                                                }}</span>
                                            <span v-if="!selectedQuarter && !selectedMonth">ทั้งปี</span></h3>
                            ...
                            <table>
                                <thead style="background-color: #F5F5F5;">
                                    <tr>
                                        ...
                                        <th class="text-h6" :colspan="priceRanges.length"
                                    ...
                                    <tr>
                                        ...
                                        <th v-for="range in priceRanges" :key="range" class="text-p"
                                    ...
                                    <tr v-for="type in dataTypes" :key="type">
                                        <td>
                                            <h6 class="text-subtitle-1">
                                                {{
                                                    type === 'unit' ? 'จำนวนหลัง' :
                                                        type === 'total_value' ? 'มูลค่ารวม' :
                                                            type === 'usable_area' ? 'พื้นที่ใช้สอย' :
                                                type === 'price_per_sqm' ? 'ราคาเฉลี่ย/ตร.ม.' : type
                                                }}
                                            </h6>
                                        </td>
                                        <td v-for="range in priceRanges" :key="type + '-' + range">
                                            <h6 class="text-subtitle-1">{{ getValue(type, range).toLocaleString('th-TH')
                                                }}</h6>
                                        </td>
                                    </tr>
                                </tbody>
          ...
          <v-col cols="12" sm="12" lg="12">
            ...
            <div class="v-row">
                <div v-for="(label, index) in ['จำนวนหลัง', 'มูลค่ารวม', 'พื้นที่ใช้สอย', 'ราคาเฉลี่ย/ตร.ม.']"
                    :key="index" class="v-col-sm-6 v-col-lg-3 v-col-12 py-0 mb-3">
                    <div
