...
interface SummaryData {
    ...
    region_data?: Record<string, any>; // Assuming you might have other data for regions
...
const fetchSummary = async (type: 'monthly' | 'quarterly') => {
    ...
    try {
        const res = await fetch('https://c44d-2405-9800-b861-96e-d38-cc71-74cd-d0c1.ngrok-free.app/package/backend/repost_admin.php', {
            method: 'POST',
...
const updateChartQuarterData = (data: SummaryData) => {
    ...
    if (selectedYears.length > 1 && selectedQuarters.length > 1) {
        selectedYears.forEach((year, index) => {
            const quarter = selectedQuarters[index]; // Mapping selected years to quarters
            const quarterIndex = parseInt(quarter); // Convert quarter to a number (1, 2, 3, 4)

            // Using optional chaining to safely access quarterly_data
            const quarterData = categoryOrder.map((category) => {
                const dataForQuarter = data.quarterly_data?.[year]?.[quarterIndex];
                return dataForQuarter && dataForQuarter[category] !== undefined ? dataForQuarter[category] : 0; // If no data, use 0
            });

            const monthsInQuarter = getMonthsInQuarter(quarterIndex); // Get the months in the selected quarter
            monthsInQuarter.forEach((monthIndex) => {
                const monthName = Months[monthIndex - 1]; // Get month name based on index
                categories.push(`${monthName} ไตรมาส ${quarter} ปี ${year}`); // Add to categories
            });

            totalValueData.push({
                name: `ไตรมาส ${quarter} ปี ${year}`, // Label for the chart
                type: 'column',
                data: quarterData // Data for the selected quarter
            });
        });
    }
    ...
    // ถ้าเลือกเพียงปีเดียวหลายไตรมาส
        ...
        selectedQuarters.forEach((quarter, index) => {
            ...
            const quarterData = categoryOrder.map((category) => {
                // Using optional chaining to avoid errors if quarterly_data is undefined
                const dataForQuarter = data.quarterly_data?.[selectedYears[0]]?.[quarterIndex];
                return dataForQuarter && dataForQuarter[category] !== undefined ? dataForQuarter[category] : 0; // Return 0 if no data
            });
    ...
    // กรณีเลือกปีเดียวและไตรมาสเดียว
        ...
        const quarterIndex = parseInt(quarter); // แปลงไตรมาสเป็นหมายเลข (1, 2, 3, 4)
        const quarterData = categoryOrder.map((category) => {
            const year = selectedYears[0]; // Use selected year
            ...
            return 0; // Return 0 if quarterly_data or the specific year is undefined
        });
...
interface SummaryData {
    ...
    region_data?: Record<string, any>; // Assuming you might have other data for regions
    quarterly_data?: Record<string, Record<number, Record<string, number>>>;
    quarterly_month_comparison?: Record<string, Record<number, Record<number, { รวม: number }>>>; // Add this property
}
...
const updateQuarterlyMonthComparisonData = (data: SummaryData) => {
    ...
    // finalChartSeries will have the structure: [{ name: "Series A", data: [val1, val2, val3] }, ...]
    ...
    selectedQuarters.sort((a, b) => a - b); // Fix sorting
    ...
    selectedYears.forEach((year) => {
        selectedQuarters.forEach((quarterNum) => {
            ...
            monthsInThisQuarter.forEach((monthIndex, positionalIndex) => {
                ...
                // Create the structure for the series if it doesn't exist
                ...
                // Store the value based on positionalIndex
                ...
                // Prepare data for each series based on positionalIndex
    ...
    // Sorting series by year, quarter, and month
...
const updateChartRegionData = (data: SummaryData) => {
    ...
    fixedRegionOrder.forEach((region) => {
        ...
        // Use optional chaining to access region_data
        const regionData = data.region_data?.[region]; // Safely access region_data
...
interface MembershipData {
    monthly_membership_data: {
        [year: string]: YearlyMembershipData;  // Ensure year is of type string
    };
}
...
// Make sure to define `chartMembershipOptions` with the correct type
const chartMembershipOptions = ref<{
 ...
 dataLabels: {
    ...
    formatter: (value: number) => {  // Explicitly define `value` as a `number`
        return value >= 1000 ? value.toLocaleString('th-TH') : value.toString();
    }
},

    xaxis: {
        type: 'category',  // Set the type to 'category' for xaxis
        categories: [],  // Explicitly set as an empty array of strings
        labels: {
            style: {
                fontSize: '12px',
                colors: '#6c757d'
                ...
                <v-col cols="12">
                    <v-card>
                        <v-card-text>
                            ...
                            <apexchart type="line" :options="chartRegionOptions" :series="chartRegionSeries" height="350" />
                        </v-card-text>
                    </v-card>
                </v-col>
                ...
                <v-col cols="12">
                    <v-card>
                        <v-card-text>
                            ...
                            <apexchart type="line" :options="chartMembershipOptions" :series="chartMembershipSeries" height="350" />
                        </v-card-text>
                    </v-card>
                </v-col>
            </v-row>
