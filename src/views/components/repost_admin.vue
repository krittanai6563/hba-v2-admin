...
const fetchSummary = async (type: 'monthly' | 'quarterly') => {
    ...
    try {
        const res = await fetch('https://d2e03fa78899.ngrok-free.app/package/backend/repost_admin.php', {
            method: 'POST',
        ...
        updateChartMembershipData(jsonData);
...
const updateChartData = (data: SummaryData) => {
    ...
    if (selectedYears.length === 1 && selectedMonths.length > 1) {
        ...
        selectedMonths.forEach((month) => {
            ...
            categories.push(`${Months[monthIndex - 1]} ${selectedYear}`);
        ...
        categories.push(...categoryOrder);

        const yearlyData = categoryOrder.map((category) => {
            return data.yearly_data[selectedYears[0]]?.[category] || 0;
        });

        totalValueData.push({
            name: `ปี ${selectedYears[0]}`,
            type: 'column',
            data: yearlyData
        });
        ...
        selectMonths.value.forEach((month) => {
            const monthIndex = monthMap[month];
            categories.push(`${Months[monthIndex - 1]} ${selectedYears[0]}`);
            ...
            totalValueData.push({
                name: `${Months[monthIndex - 1]} ${selectedYears[0]}`,
                type: 'column',
                data: monthlyData1
            });

            categories.push(`${Months[monthIndex - 1]} ${selectedYears[1]}`);
            ...
            totalValueData.push({
                name: `${Months[monthIndex - 1]} ${selectedYears[1]}`,
                type: 'column',
                data: monthlyData2
            });
        });
        ...
        selectedYears.forEach((year) => {
            const yearlyData = categoryOrder.map((category) => {
                return data.yearly_data[year]?.[category] || 0;
            });
...
interface MembershipData {
    monthly_membership_data: {
        [year: string]: YearlyMembershipData;
    };
}
...
const updateChartMembershipData = (data: SummaryData) => {
    ...
    baseMonths.forEach((baseMonth, baseMonthIndex) => {
        let categoryLabel = Months[baseMonth - 1];
...
const exportAllGraphsToPDF = () => {
    ...
    const titleText = 'มูคค่ายอดเซ็นสัญญา ของสมาชิกสมาคม ';
    doc.setFontSize(35);
    ...
    const subtitleText = 'ที่มา : สมาชิกสมาคมธุรกิจรับสร้างบ้าน (HBA)';
    doc.setFontSize(20);
    ...
    const chartTitles = [
        'กราฟเปรียบเทียบยอดเซ็นสัญญา แยกตามระดับราคาบ้าน(รายเดือน)',
        'กราฟเปรียบเทียบยอดเซ็นสัญญา แยกตามระดับราคาบ้าน(ไตรมาส)',
        'กราฟเปรียบเทียบยอดเซ็นสัญญา แยกตามภูมิภาค',
        'กราฟเปรียบเทียบข้อมูลสมาชิกกรอกยอดเซ็นสัญญา'
    ];

    const chartSubtitles = [
        String(chartSubtitle.value),
        String(chartQuarterSubtitle.value),
        String(chartRegionSubtitle.value),
        String(chartMembershipSubtitle.value)
    ];

    chartTitles.forEach((title, index) => {
        const chartElement = document.querySelector(`#chart${index + 1}`) as HTMLElement;
        ...
        if (chartElement) {
            html2canvas(chartElement).then((canvas) => {
                ...
                doc.setFontSize(25);
                const titleWidth = doc.getTextWidth(chartTitles[index]);
                const titleX = (doc.internal.pageSize.width - titleWidth) / 2;
                doc.text(chartTitles[index], titleX, currentYPosition);
                doc.setFontSize(19);
                const subtitleWidth = doc.getTextWidth(chartSubtitles[index]);
                const subtitleX = (doc.internal.pageSize.width - subtitleWidth) / 2;
                doc.text(chartSubtitles[index], subtitleX, currentYPosition + 10);

                currentYPosition += 40;

                doc.addImage(imgData, 'PNG', margin, currentYPosition, newWidth, newHeight);

                currentYPosition += newHeight + 20;

                if (index === chartTitles.length - 1) {
                    doc.save('report-all-charts.pdf');
                }
            });
        }
    });
                ...
                <v-col cols="12">
                    <v-card>
                        <v-card-text>
                            ...
                            <v-btn @click="exportAllGraphsToPDF" color="primary"> ดาวน์โหลด PDF </v-btn>
                        </v-card-text>
                    </v-card>
                </v-col>
