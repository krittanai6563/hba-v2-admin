...
const fetchSummary = async () => {
  ...
  try {
    const res = await fetch(`http://localhost/package/backend/roport_pricerange.php`, {
      method: 'POST',
    ...
    summaryData.value = { yearly_data: {}, quarterly_data: {} };
  } finally {
...
const series64 = computed(() => {
  ...
  // ตรวจสอบว่า 'yearly_data_quarterly_region' ไม่เป็น undefined ก่อนเข้าถึงข้อมูล
  const yearlyData = summaryData.value?.yearly_data_quarterly_region;
  if (!yearlyData) {
    console.error('Data for regions is missing');
    return [];  // ส่งคืนข้อมูลว่างหากไม่พบข้อมูล
  }

  // ใช้ selectedYear และ selectedComparisonPeriod เพื่อเลือกปีเปรียบเทียบ
  const year1 = selectedYear.value;  // ปีที่เลือกจากฟอร์ม
  const year2 = selectedComparisonPeriod.value;  // ปีที่เลือกจากฟอร์มสำหรับการเปรียบเทียบ
  const quarter = selectedComparisonQuarter.value;  // ไตรมาสที่เลือกจากฟอร์ม

  // ดึงข้อมูลของแต่ละภูมิภาคในไตรมาสที่เลือกจากปีที่เลือก
  const regionData1 = regions.map(region => yearlyData[region]?.[year1]?.[quarter] || 0);  // ใช้ไตรมาสที่เลือกจากปีแรก
  const regionData2 = regions.map(region => yearlyData[region]?.[year2]?.[quarter] || 0);  // ใช้ไตรมาสที่เลือกจากปีที่สอง
...
const memberSeries = computed(() => {
  ...
  const yearTotalMembers = monthsInQuarter.map(month =>
    parseInt(data.find(item => item.month_number === month && item.year === selectedYear)?.total_members || 0) // สมาชิกทั้งหมด
  );

  const yearSubmittedMembers = monthsInQuarter.map(month =>
    parseInt(data.find(item => item.month_number === month && item.year === selectedYear)?.submitted_members || 0) // สมาชิกที่กรอกข้อมูล
  );

  const yearNotSubmittedMembers = monthsInQuarter.map(month =>
    parseInt(data.find(item => item.month_number === month && item.year === selectedYear)?.not_submitted_members || 0) // สมาชิกที่ยังไม่ได้กรอกข้อมูล
  );
    ...
    <v-col cols="12">
      <v-card>
        <v-card-text>
          ...
          <h5 class="card-subtitle">สามารถเลือกข้อมูลสำหรับการค้นหา</h5><br>

          <v-row>
            <!-- เลือกปี -->
            <v-col cols="12" md="4">
              <v-select v-model="selectedYear" :items="years" label="เลือกปี" item-text="year" item-value="year"
                outlined></v-select>
            </v-col>

            <!-- เลือกเดือน -->
            <v-col cols="12" md="4">
              <v-select v-model="selectedMonth" :items="months" label="เลือกเดือน" item-text="text" item-value="value"
                outlined></v-select>
            </v-col>

            <v-col cols="12" md="4">
              <v-select v-model="selectedComparisonQuarter" :items="Quarter" label="เลือกไตรมาส" item-text="text"
                item-value="value" outlined />
            </v-col>
            <v-col cols="12" md="4">
              <v-select v-model="selectedComparisonType" :items="comparisonTypes" label="เลือกประเภทการเปรียบเทียบ"
                outlined></v-select>
            </v-col>

            <!-- เลือกช่วงเวลาในการเปรียบเทียบ -->
            <v-col cols="12" md="4">
              <v-select v-model="selectedComparisonPeriod" :items="selectedComparisonPeriods"
                label="เลือกช่วงเวลาในการเปรียบเทียบ" :disabled="!selectedComparisonType" outlined></v-select>
            </v-col>
        ...
        <v-card-text>
          <div>
            ...
            <h5 class="card-subtitle">
              เดือน {{ selectedMonth }} ปี ({{ selectedYear }} - {{ selectedComparisonPeriod }})
            </h5>
          </div>
    ...
    <v-col cols="12">
      <v-card>
        <v-card-text>
          ...
          <apexchart height="350" :options="chartOptions44" :series="series64"></apexchart> <!-- แสดงกราฟ -->
        </v-card-text>
      </v-card>
    </v-col>
  </v-row>
